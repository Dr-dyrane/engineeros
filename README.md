...
- **Career Launchpad** — the last mile to a job. An application tracker (company, role, status pipeline, next action, notes) so the search is a visible, running thing, plus interview prep: role-specific questions with a STAR answer builder that drafts from the portfolio projects you already wrote, and an exportable prep sheet.
